...
export default function (data) {
    ...
    try {
        ...
        if (operation < 0.6) {
            let res = http.get(`${BASE_URL}/detalle-pedido/${PEDIDO_ID}`, { headers: sessionHeaders });
            check(res, { 'GET lista': (r) => r.status === 200 });
        } else {
            let res = http.get(`${BASE_URL}/detalle-pedido/nuevo/${PEDIDO_ID}`, { headers: sessionHeaders });
